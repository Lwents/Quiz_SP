...
- Default settings in ackend/app/core/config.py, docker-compose.yml, and rontend/ MUST always use safe local dummy defaults (e.g. http://localhost:8000/v1, "", change-this-...).
- All real credentials MUST be loaded strictly via environment variables from .env or ackend/.env.
...
### 3. File Protection
- .env, .env.*, ackend/.env, and any credentials MUST remain in .gitignore.
